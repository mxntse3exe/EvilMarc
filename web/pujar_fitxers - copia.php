    ...
    if (isset($_FILES['archivo'])) {
        ...
        foreach ($_FILES['archivo']['name'] as $key => $name) {
            ...
            if ($error === UPLOAD_ERR_OK) {
                $file_path = $current_dir . DIRECTORY_SEPARATOR . $name;

                // Si es un directorio, crea la carpeta
                if (is_dir($tmp_name)) {
                    if (!file_exists($file_path)) {
                        mkdir($file_path, 0777, true);
                    }
                } else {
                    // Si es un archivo, muévelo al directorio correspondiente
                ...
                <div style="width: 90%;">
                    ...
                    <!-- <form enctype="multipart/form-data" method="post" action="pujar_fitxers" class="pujar_arxius_carpetes">
                        ...
                        <label for="archivo" class="custom-file-upload">
                            <i class="uil uil-file-alt"></i>Selecciona l'arxiu o carpeta que vols pujar...
                        </label>
                        <input type="file" name="archivo[]" class="input_arxiu" id="archivo" onchange="mostrarNombreArchivo()" multiple webkitdirectory>
                        <input type="submit" value="Pujar arxiu" class="pujar_arxiu_submit" name="archivo">
                    </form>                     -->
